...
class Workshop extends CI_Controller {
	...
	{
		$jumlah= $this->Crud_m->view_where('workshop',array('workshop_status'=>'publish'))->num_rows();
		$config['base_url'] = base_url().'workshop/index/page/';
		...
		if (is_numeric($dari)) {
			...
			$data['posts'] = $this->Crud_m->view_where_orderings('workshop',array('workshop_status'=>'publish'),'workshop_id','DESC');
		...
		$this->load->view('frontend/v_workshop',$data);
	}
}
